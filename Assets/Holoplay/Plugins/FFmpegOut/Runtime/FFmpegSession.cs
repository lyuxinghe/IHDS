...
namespace FFmpegOut
{
    public sealed class FFmpegSession : System.IDisposable
    {
        ...
        public static FFmpegSession Create(
            ...
            FFmpegPreset preset, string extraFfmpegOptions = ""
        )
        {
            name += System.DateTime.Now.ToString(" yyyy MMdd HHmmss").Replace(" ", "_");
            var path = name + preset.GetSuffix();
            return CreateWithOutputPath(path, width, height, frameRate, preset, extraFfmpegOptions);
        }

        public static FFmpegSession CreateWithOutputPath(
            string outputPath,
            int width, int height, float frameRate,
            FFmpegPreset preset, string extraOptions = ""
        )
        {
            ...
            // Debug.Log("new ffmpeg session: " + outputPath);
